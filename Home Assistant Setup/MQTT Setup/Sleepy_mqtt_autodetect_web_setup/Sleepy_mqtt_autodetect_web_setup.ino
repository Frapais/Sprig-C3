...
const unsigned long BOOT_HOLD_MS = 3000; // hold time (ms) for factory reset
// Deep sleep settings
const uint32_t SLEEP_MINUTES = 10; // minutes to deep sleep after successful publish

bool configPortalActive = false; // true when running AP/web server for config
...
void setup() {
  ...
  // Check for BOOT long-press at startup: force config portal (factory reset)
  unsigned long btnStart = millis();
  if (digitalRead(BOOT) == LOW) {
    while (digitalRead(BOOT) == LOW && (millis() - btnStart) < BOOT_HOLD_MS) {
      delay(10);
    }
    if (digitalRead(BOOT) == LOW) {
      // Held long enough: clear stored prefs and start config portal
      prefs.clear();
      prefs.end();
      startConfigPortal();
      return;
  ...
  // attempt normal WiFi connection with timeout
  if (!setup_wifi_with_timeout(20000)) {
    // failed to connect -> start config portal
    startConfigPortal();
    return;
  }
  ...
  // Attempt MQTT connection with limited retries. If connected, publish discovery and battery then deep sleep.
  int tries = 0;
  const int maxMqttTries = 3;
  ...
  bool mqttOk = false;
  while (tries < maxMqttTries) {
    Serial.print("Attempting MQTT connection (try "); Serial.print(tries+1); Serial.println(")...");
    if (MQTT_client.connect(clientId.c_str(), mqttUserStr.c_str(), mqttPassStr.c_str())) {
      mqttOk = true;
      Serial.println("MQTT connected");
      break;
    }
    Serial.print("MQTT failed, rc="); Serial.println(MQTT_client.state());
    tries++;
    delay(2000);
  }

  if (!mqttOk) {
    Serial.println("Could not connect to MQTT -> starting config portal");
    startConfigPortal();
    return;
  }

  // We're connected to MQTT: subscribe and publish discovery
  MQTT_client.subscribe(discoverySetTopic);
  haDiscovery();
  MQTT_client.publish(discoveryStateTopic, (auto_discovery ? "ON" : "OFF"), true);

  // Read battery once and publish
  float batVoltage = maxlipo.cellVoltage();
  char voltString[16];
  dtostrf(batVoltage, 1, 3, voltString);
  MQTT_client.publish("sprigC3/batVoltage", voltString, true);

  float batLevel = maxlipo.cellPercent();
  char levString[16];
  dtostrf(batLevel, 1, 1, levString);
  MQTT_client.publish("sprigC3/batLevel", levString, true);

  // allow some time for network traffic to flush
...
// Try to connect to WiFi but with a timeout (ms). Returns true if connected.
bool setup_wifi_with_timeout(uint32_t timeoutMs) {
  delay(10);
  Serial.print("Connecting to ");
  Serial.println(cfg_wifi_ssid.length() ? cfg_wifi_ssid : String(SSID));
  if (cfg_wifi_ssid.length()) WiFi.begin(cfg_wifi_ssid.c_str(), cfg_wifi_pass.c_str());
...
void loop() {
  ...
  if (cfg_wifi_ssid.length() == 0 || configPortalActive) {
    server.handleClient();
    delay(2);
    return;
  }
